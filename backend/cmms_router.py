...
TEMPLATES = {
    ...
    "MaintainX": {"base_url": "", "field_mappings": {"work_order_number": "id", "title": "title", "status": "status"}, "operations": {"create_work_order": "/work-orders", "update_status": "/work-orders/{id}", "close_job": "/work-orders/{id}", "fetch_asset_history": "/assets/{id}"}},
    "MP2": {"base_url": "https://YOUR-MP2-HOST/api", "field_mappings": {"work_order_number": "workOrderNumber", "title": "description", "status": "status"}, "operations": {"create_work_order": "/workorders", "update_status": "/workorders/{id}", "close_job": "/workorders/{id}", "fetch_asset_history": "/assets/{id}"}},
...
async def save_config(organization_id: str, payload: CmmsConfigInput, user: CurrentUser):
    ...
    if payload.provider not in TEMPLATES:
        raise HTTPException(status_code=422, detail="Choose a supported CMMS template.")
    if payload.base_url and not payload.base_url.startswith("https://"):
        raise HTTPException(status_code=422, detail="CMMS endpoints must use HTTPS.")
...
async def preview_operation(organization_id: str, operation: str, user: CurrentUser):
    await require_organization(user, organization_id, "cmms:manage")
    config = await db.cmms_configs.find_one({"organization_id": organization_id}, {"_id": 0})
    if not config:
        raise HTTPException(status_code=404, detail="Save a CMMS configuration first.")
    endpoint = TEMPLATES[config["provider"]]["operations"].get(operation)
    if not endpoint:
        raise HTTPException(status_code=422, detail="That operation is unavailable for this template.")
    return {"operation": operation, "provider": config["provider"], "base_url": config["base_url"], "endpoint": endpoint, "field_mappings": config["field_mappings"], "ready_for_test": bool(config.get("api_key_encrypted"))}
